...
def plot_hist(dataset, marks):
    for subject_school in marks.columns.values:
        plt.figure()
        plt.hist(get_marks_by_house(dataset, marks, 'Gryffindor', subject_school), bins=50, label = 'Gryffindor', color = 'r')
        ...
        plt.hist(get_marks_by_house(dataset, marks, 'Hufflepuff', subject_school), bins=50,label = 'Hufflepuff', color = 'y')
